...
├── LICENSE                # 许可证文件（发布时不包含）
├── popup.html              # 扩展程序弹出页面
├── PRIVACY.md             # 隐私政策文件（发布时不包含）
├── css/                   # 样式文件夹
│   ├── common.css         # 通用样式
...
可以使用我提供的docker测试文件进行本地测试。使用方法如下：
...
**本扩展目前由作者独立开发，难免存在考虑不周或不尽如人意之处，敬请理解。也诚挚欢迎各位同学在项目 Issue 区提出宝贵建议，与我一同打磨细节，让南京大学的选课体验不断优化、日益完善。**
